...
def setdsetandload(datasetused):
    ...
    df=pd.read_csv("ProductsCleanDatasets/"+datasetused+".csv")
...
for filename in os.listdir(directory):
    ...
    if os.path.isfile(f):
        ...
        recieved  =  setdsetandload(filename)
...
def Ultimate_out( datasetname ):
    ...
    future_df['Booked_Qty forecast'] = new_results.predict(start = len(df)-1, end = 144, dynamic= True)  
